...
def generate_subtitles_clip(subs, delay=0.05):
    ...
    return SubtitlesClip(clips, lambda txt: TextClip(txt, fontsize=100, color='yellow', method='caption', stroke_color="black", stroke_width=6, font="Impact"))


# Load your video clip
def generate_video(video_p, count, start_time, end_time):
    video_path = video_p
    video = VideoFileClip(video_path).subclip(start_time, end_time).fx(vfx.fadeout, 1)
    ...
    final_video = CompositeVideoClip([video.set_duration(subs_clip.duration), subs_clip.set_position(((1920/2 - 1080/2 + 40) , 1200))])

    # Write the final video file
    final_video.write_videofile(f"beta_video{count}.mp4", fps=24)
count = 5
while True:
    ...
    generate_video(file_name, count, start, end)
    count += 1 
    if link == "exit":
        break